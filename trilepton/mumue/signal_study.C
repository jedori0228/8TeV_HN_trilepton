...
void signal_study(){
  ...
  chain_mu = new TChain("Muon");
  chain_el = new TChain("Electron");
  chain_gen = new TChain("Gen");
  chain_event = new TChain("Event");
  chain_mu->Add("./nocut/signal/trilepton_mumue_SKHN"+TString::Itoa(sample, 10)+"_lep_5_3_14.root");
  chain_el->Add("./nocut/signal/trilepton_mumue_SKHN"+TString::Itoa(sample, 10)+"_lep_5_3_14.root");
  chain_gen->Add("./nocut/signal/trilepton_mumue_SKHN"+TString::Itoa(sample, 10)+"_lep_5_3_14.root");
  chain_event->Add("./nocut/signal/trilepton_mumue_SKHN"+TString::Itoa(sample, 10)+"_lep_5_3_14.root");
  ...
  double MET, METphi, nu_Pz, x, W_mass_fit, MET_fit, weight[N_chain];
  int N_entry;
  ...
  N_entry = 0;
  mu = new Muon(chain_mu[k], 2);
  el = new Electron(chain_el[k], 1);
  gen = new Gen(chain_gen);
  evt = new Event(chain_event[k]);
  
  hist_HN = new TH1D("hist_HN_"+chainlist, "HN invariant mass", 30./1., 40., 70.);
  hist_W_real = new TH1D("hist_W_real_"+chainlist, "W_real invariant mass", 30./1., 70., 100.);
  hist_l1l2 = new TH1D("hist_l1l2_"+chainlist, "l1l2 invariant mass", 200./5., 0., 200.);
  hist_l2l3 = new TH1D("hist_l2l3_"+chainlist, "l2l3 invariant mass", 200./5., 0., 200.);
  
  cout << endl << "Running Sample : " << chainlist << endl;
  
  
  for(int i=0; i<chain_mu->GetEntries(); i++){
    
    printeventnumber(i, chain_mu->GetEntries());
    
    chain_mu->GetEntry(i);
    chain_event->GetEntry(i);
    ...
    if( mu[k]->isHNLooseMuon(0) && mu[k]->isHNLooseMuon(1) ){
      ...
      //if( 1 ){ // NoHNLoose cut
      lep[0].SetPxPyPzE(mu[k]->Px[0],mu[k]->Py[0],mu[k]->Pz[0],mu[k]->E[0]);
      lep[1].SetPxPyPzE(el[k]->Px[0],el[k]->Py[0],el[k]->Pz[0],el[k]->E[0]);
      lep[2].SetPxPyPzE(mu[k]->Px[1],mu[k]->Py[1],mu[k]->Pz[1],mu[k]->E[1]);
      ...
      if(fitresult_p.chi2 < 1.0 && fitresult_p.lambda_fit == 0){  // chi2 < 1 && lambda_fit == 0 cut applied
        ...
        hist_l1l2[k]->Fill( ( lep[0] + lep[1] ).M() );
        hist_l2l3[k]->Fill( ( lep[1] + lep[2] ).M() );
        
      } // chi2 < 1 cut
    } // Loosemuon cut
  } // event loop
  
  if(!isPU) weight[k] = evt[k]->weight;
  if(isPU)  weight[k] = evt[k]->weight * evt[k]->PU_reweight; // pileup weight
  cout << "weight = " << weight[k] << endl
  << "entry = " << N_entry << endl;
  
  hist_HN[k]->Scale(weight[k]);
  hist_W_real[k]->Scale(weight[k]);
  hist_l1l2[k]->Scale(weight[k]);
  hist_l2l3[k]->Scale(weight[k]);
  ...
  if(sample == 40)  lg1->AddEntry(hist_HN[0], "signal", "l");
  ...
  bkgstack_W_real->SetMaximum(1500);
  ...
  if(sample == 40)  lg2->AddEntry(hist_W_real[0], "signal", "l");
